...
connection.onCompletion(
  (_textDocumentPosition: TextDocumentPositionParams): CompletionItem[] => {
    ...
    let matchArray: RegExpMatchArray | null = line.match(regex)
    if (!matchArray || !matchArray.groups)
      return CompletionItems
    // not needed yet, but maybe you want to use these two variables some day.
    // let wholeMatch = matchArray[0]
    // let type = matchArray.groups['type']
    let subtype = matchArray.groups['subtype'];
    let tempRec: boolean = matchArray.groups['temp'] !== undefined
